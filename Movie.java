 ...
 * It consists of two constructors.
 * Each Movie is required(required to create a Movie Object) to have the following:
 * 	- Title
 * 	- Year
 * The following is optional for each Movie:
 * 	- Director
 * 	- Writer
 * 	- Actors
 * 
...
public class Movie implements Comparable<Movie> {
	 ...
	 * @param year year that the Movie was released; should be in the range of 1900 to 2020
	...
	/**
	 * Constructs a new Movie object with specified title, year, director, writer, actor1, actor2, and actor3
	 * @param title title of the Movie; cannot be null or an empty string
	 * @param year year that the Movie was released; should be in the range of 1900 to 2020
	 ...
	 * @throws IllegalArgumentException if the title, year, or actor1 parameters are invalid
	 */
	...
	/**
	 * Searches through the current locations of this Movie object and if the location parameter
	 * is not in the list of locations, then it will be added to the locations of this Movie object
	...
	public boolean equals(Object o) {
		if (o instanceof Movie) {
			...
			if (this.compareTo(comparedMovie) == 0) {
				return true;
			}
		}
	...
	public String toString() {
		...
		// append different lines depending on how many Actors are in this Movie
		 if (this.getActorList().size() == 1) {
			...
			formattedMovieStringBuilder.append(String.format("%-15s : %s, %s%n", "starring",this.actor1.getName(), this.actor2.getName()));
		}
		else if (this.getActorList().size() == 3) {
			formattedMovieStringBuilder.append(String.format("%-15s : %s, %s, %s%n", "starring",this.actor1.getName(),this.actor2.getName(),this.actor3.getName()));
		}
		
		formattedMovieStringBuilder.append("filmed on location at: \n");
		
		// append different lines depending on if each loc in this Movie has a fun fact or not
		for (Location loc:this.getLocations()) {
			if (loc.getFunFacts() != null && !loc.getFunFacts().isEmpty()) {
				formattedMovieStringBuilder.append(String.format("    %s (%s)%n", loc.getLocation(),loc.getFunFacts()));
			}
			else {
