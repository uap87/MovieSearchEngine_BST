...
/**
 * This class is the program parses input file and returns user queries.
 * The program is interactive. 
 * When the program is executed the name of the input file containing the list of all the movies
 * is provided as the program's single command line argument. The data in this file 
 * serves as a database of all the movies that are considered valid. 
 * In the interactive part, the user enters "title" or "actor" followed by a keyword. The program 
 * responds by printing the movies with their description that contain either the keyword in the "title" or "actor"
 * depending on the input (if at least one movie exists that meets the conditions).
 * 
 * @author Udit Patel
 * @version 11/5/2018
 */
public class SFMovieData {
	...
	public static void main(String[] args) {
		...
		// create and instantiate MovieList of movies in database, line that will be read, and ArrayList of movieData for each line read
		...
		while (inFile.hasNextLine()) {
			...
			// try reading through lines, parsing them, and adding them to the Movie database given they pass the conditions
			try {
				...
				// if movie does not exist in database, go through conditions in order to add movie
				if (!containsMovie) {
					...
					// if actor1 object is not null and location value is not null or not an empty String,
					// create Movie object using parameters, and add location to the movie
					if (actor1 != null) {
						if ((movieData.get(2) != null) && (!movieData.get(2).isEmpty())) {
							newMovie = new Movie(movieData.get(0),Integer.parseInt(movieData.get(1)),movieData.get(6),movieData.get(7),actor1,actor2,actor3);
				...
				// if movie exists in database and if location value is not null and not an empty String,
				// add location to the existing Movie object in the database
				else {
					if ((movieData.get(2) != null) && (!movieData.get(2).isEmpty())) {
						newMovie.addLocation(new Location(movieData.get(2),movieData.get(3)));
					}
		...
		while(repeat) {
			...
			// else if user enters "title KEYWORD", call getMatchingTitles method and print matching Movies, if there are any
			...
			// else if user enters "actor KEYWORD", call getMatchingActor method and print matching Movies, if there are any
